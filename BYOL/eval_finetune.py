...
from main_knowledge import RandomFlip, RandomSwap, RandomMotion, seed_everything
...
class LogisticRegression(torch.nn.Module):
    def __init__(self, input_dim, output_dim):
        ...
        self.apply_init()
        tmp = [self.batch, self.linear, self.relu, self.linear_2]
        self.seq = nn.Sequential(*tmp)
...
def main(gpu, args):
    ...
    for fold in range(5):
        ...
        if 'online_network_1_state_dict' in load_params and 'shared_online_state_dict' in load_params:
            encoder.load_state_dict(load_params['online_network_1_state_dict'])
            if 'Conv' in args.model:
                shared_encoder.load_state_dict(load_params['shared_online_state_dict'])
            print("Parameters successfully loaded.")
        # remove the projection head
        if args.model == 'resnet' or 'vit' in args.model:
            encoder = torch.nn.Sequential(*list(encoder.children())[:-1])
        else:
            encoder = connect_net(encoder, shared_encoder)
        # default output_feature_dim = 512
        logreg = LogisticRegression(output_feature_dim, 2)

        model = final(encoder, logreg)
        ...
        optimizer = torch.optim.SGD(logreg.parameters(), lr=0.0001, momentum=0.9,
                                    weight_decay=0)
        ...
        for epoch in range(args.max_epochs):
            ...
            for batch_data in train_loader:
                x = batch_data['img_mri'].to(args.device)
