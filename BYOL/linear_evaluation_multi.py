...
def get_features_from_encoder(args, encoder_1, encoder_2, loader, device):
    x_train = []
    y_train = []
    ...
    for step, batch_data in enumerate(loader):
        with torch.no_grad():
            feature_vector_1 = encoder_1(batch_data[f'img_{args.modality[0]}'].to(device))
            feature_vector_2 = encoder_1(batch_data[f'img_{args.modality[1]}'].to(device))
            x_train.extend(torch.concat((feature_vector_1, feature_vector_2), dim=1))
...
def main(gpu, args):
    ...
    for fold in range(5):
        ...
        test_loader = DataLoader(val_dataset, batch_size=args.batch_size,
                                 num_workers=args.num_workers, drop_last=False, shuffle=True)
        encoder_1 = Conv5_FC3().convolutions
        encoder_2 = Conv5_FC3().convolutions
        ...
        if 'online_network_state_dict' in load_params:
            ...
            encoder_1.load_state_dict(load_params['online_network_1_state_dict'])
            encoder_2.load_state_dict(load_params['online_network_2_state_dict'])
            print("Parameters successfully loaded.")

        # default output_feature_dim = 512
        logreg = LogisticRegression(output_feature_dim, 2)
        if torch.cuda.device_count() > 1:
            encoder_1 = nn.DataParallel(encoder_1)
            encoder_2 = nn.DataParallel(encoder_2)
            logreg = nn.DataParallel(logreg)
        encoder_1 = encoder_1.to(args.device)
        encoder_2 = encoder_2.to(args.device)
        logreg = logreg.to(args.device)

        encoder_1.eval()
        encoder_2.eval()
        x_train, y_train = get_features_from_encoder(args, encoder_1, encoder_2, train_loader, args.device)
        x_test, y_test = get_features_from_encoder(args, encoder_1, encoder_2, test_loader, args.device)
        ...
        optimizer = torch.optim.SGD(logreg.parameters(), 0.0001, momentum=0.9,
                                    weight_decay=0)
