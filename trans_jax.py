...
def bezier_curve(points, nTimes=1000):
    ...
    nPoints = len(points)
    xPoints = jnp.array([p[0] for p in points])
    yPoints = jnp.array([p[1] for p in points])

    t = jnp.linspace(0.0, 1.0, nTimes)

    polynomial_array = jnp.array([bernstein_poly(i, nPoints - 1, t) for i in range(0, nPoints)])
...
def data_augmentation(x, y, prob=0.5):
    ...
    while random.random() < prob and cnt > 0:
        ...
        x = jnp.flip(x, axis=degree)
        y = jnp.flip(y, axis=degree)
        cnt = cnt - 1
...
def nonlinear_transformation(x, prob=0.5):
    ...
    if random.random() < 0.5:
        ...
        xvals, yvals = jnp.sort(xvals), jnp.sort(yvals)
    nonlinear_x = jnp.interp(x, xvals, yvals)
    return nonlinear_x
...
def local_pixel_shuffling(x, prob=0.5):
    ...
    for _ in range(num_block):
        ...
        noise_y:noise_y + block_noise_size_y,
        noise_z:noise_z + block_noise_size_z] = window.copy()
...
def image_in_painting(x):
    ...
    while cnt > 0 and random.random() < 0.95:
        ...
        x = x.at[:,
            ...
            noise_y:noise_y + block_noise_size_y,
            noise_z:noise_z + block_noise_size_z].set(np.random.rand(block_noise_size_x,
                                                                     block_noise_size_y,
...
def image_out_painting(x):
    ...
    while cnt > 0 and random.random() < 0.95:
        ...
        noise_x:noise_x + block_noise_size_x,
        noise_y:noise_y + block_noise_size_y,
        noise_z:noise_z + block_noise_size_z].set(image_temp[:, noise_x:noise_x + block_noise_size_x,
                                                noise_y:noise_y + block_noise_size_y,
                                                noise_z:noise_z + block_noise_size_z])
        cnt -= 1
    return x
