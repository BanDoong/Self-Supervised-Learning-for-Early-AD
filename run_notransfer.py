...
def cal_metric(cf_list):
    ...
    accuracy, specificity, sensitivity, F1, precision = to_numpy(accuracy, specificity, sensitivity, F1, precision)
    bacc = bacc.numpy()
    return accuracy, bacc, specificity, sensitivity, F1, precision
...
def main(gpu, args):
    ...
    for fold in range(5):
        ...
        from simclr import SimCLR_notransfer
        ...
        if 'resnet' == args.model:
            ...
            model = SimCLR_notransfer(encoder, args)
        ...
        save_model(args, model, optimizer)
        result_df.to_csv(f'{args.model_path}/{args.modality}_mci_{args.mci}_epochs_{args.epoch_num}_results_fold_{fold}.csv')
