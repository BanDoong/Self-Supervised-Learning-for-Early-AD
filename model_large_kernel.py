...
class RepLKNet(nn.Module):
    ...
    def __init__(self, large_kernel_sizes, layers, channels, drop_path_rate, small_kernel,
                 dw_ratio=1, ffn_ratio=4, in_channels=1, num_classes=2, out_indices=None,
    ...
    def deep_fuse_BN(self):
        for m in self.modules():
            ...
            if hasattr(m[0], 'kernel_size') and hasattr(m[0], 'weight') and isinstance(m[1], nn.BatchNorm3d):
                conv = m[0]
                bn = m[1]
...
def create_RepLKNet31B(drop_path_rate=0.3, num_classes=2, use_checkpoint=True, small_kernel_merged=False):
